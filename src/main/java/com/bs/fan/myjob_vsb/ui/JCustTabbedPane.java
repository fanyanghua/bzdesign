...
public class JCustTabbedPane extends JTabbedPane implements ApplicationContextAware {
     ...
     * @param
     */
    public JCustTabbedPane(boolean arg0) {
        super();
        showClose = arg0;
        initialize();
    }
                ...
                public void mouseMoved(MouseEvent e) {
                    ...
                    if (closable.size() > 0) {
                        ...
                        if (oldIndex != nowIndex) {
                            if (nowIndex != -1) {
                                repaint(closeRects[nowIndex]);// �����ػ�����
                            } else {
                                if (oldIndex < getTabCount()) {
                                    repaint(closeRects[oldIndex]);// �����ػ�����
                                }
                            }
                            oldIndex = nowIndex;
                        }
...
//            paintCloseIcon(g2d, tabIndex, isSelected);
    ...
    public void addOrSelectTab(String header, Icon icon, Class cls, String tip, boolean closable) {
        //确认是否已添加该tab
        int tabCount = getTabCount();
        for (int i = 0; i < tabCount; i++)
            if (getTitleAt(i).equals(header)) {//已添加该tab,则选择.
                setSelectedIndex(i);
                return;
            }
        Component component = (Component) applicationContext.getBean(cls);

        addTab(header, icon, component, tip, closable);
